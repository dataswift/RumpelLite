...
internal struct HappinessAndMentalCachingWrapperHelper {
    ...
    static func getSurveyObject(userToken: String, userDomain: String, cacheTypeID: String, successRespond: @escaping ([SurveyObject], String?) -> Void, failRespond: @escaping (HATTableError) -> Void) {
        ...
        HappinessAndMentalCachingWrapperHelper.checkForUnsyncedSurveyObjectToUpdate(
            userDomain: userDomain,
            userToken: userToken)
        ...
        AsyncCachingHelper.decider(
            ...
            value: 1,
            networkRequest: HappinessAndMentalCachingWrapperHelper.requestSurveyObject(userToken: userToken, userDomain: userDomain, failRespond: failRespond),
    ...
    static func postSurveyObject(surveyObjects: [SurveyObject], userToken: String, userDomain: String, successCallback: @escaping () -> Void, errorCallback: @escaping (HATTableError) -> Void) {
        ...
        CachingHelper.deleteFromRealm(type: "happinessAndMentalHealth")
        CachingHelper.deleteFromRealm(type: "happinessAndMentalHealth-Post")
        ...
        do {
            ...
            try realm.write {
                ...
                let jsonObject = JSONCacheObject(dictionary: array, type: "happinessAndMentalHealth", expiresIn: .day, value: 1)
                realm.add(jsonObject)
                
                let jsonObject2 = JSONCacheObject(dictionary: array, type: "happinessAndMentalHealth-Post", expiresIn: nil, value: nil)
    ...
    static func checkForUnsyncedSurveyObjectToUpdate(userDomain: String, userToken: String, completion: (() -> Void)? = nil, errorCallback: ((HATTableError) -> Void)? = nil) {
        ...
        CheckCache.searchForUnsyncedCache(type: "happinessAndMentalHealth-Post", sync: tryUpdating)
    }
}
