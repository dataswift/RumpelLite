...
internal class ShareOptionsViewController: UIViewController, UITextViewDelegate, PhotoPickerDelegate, UICollectionViewDataSource, UICollectionViewDelegate, SendLocationDataDelegate, UserCredentialsProtocol, SelectedPhotosProtocol {
    ...
    private func getDataPlugs() {
        ...
        func gotPlugs(dataPlugs: [HATDataPlugObject], newToken: String?) {
            
            self.dataPlugs = HATDataPlugsService.filterAvailableDataPlugs(dataPlugs: dataPlugs)
        }
        
        HATDataPlugsService.getAvailableDataPlugs(succesfulCallBack: gotPlugs, failCallBack: { _ in return })
    }
    // MARK: - Autogenerated
